...
class GroupJoinViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func observeFirebaseMessages() {
        ref.observe(.childAdded) { [weak self] snapshot in
            ...
            let newMessage = GJChatMessage(text: text, isIncoming: true, sender: senderName, senderID: senderID)
    ...
    @IBAction func didTapMicButton(_ sender: UIButton) {
        ...
        if !isRecording {
            ...
            do {
                ...
                // UI Updates
                DispatchQueue.main.async {
                    self.GJmicButton.tintColor = .systemRed
                    self.GJmicButton.setImage(UIImage(systemName: "mic.fill"), for: .normal)
                }
            ...
            DispatchQueue.main.async {
                self.GJmicButton.tintColor = .systemBlue
                self.GJmicButton.setImage(UIImage(systemName: "mic"), for: .normal)
            }
    ...
    func startLiveTranscription() {
        let newMessage = GJChatMessage(text: "Listening...", isIncoming: false, sender: self.myName, senderID: self.currentUserID)
        self.messages.append(newMessage)
        
        let lastIndex = IndexPath(item: self.messages.count - 1, section: 0)
        self.GJcollectionView.insertItems(at: [lastIndex])
        self.scrollToBottom()
        ...
        speechManager.startTranscribing(languageCode: self.selectedLanguageCode) { [weak self] transcribedText in
            ...
            self.messages[indexToUpdate] = GJChatMessage(
            ...
            DispatchQueue.main.async {
                UIView.performWithoutAnimation {
                    self.GJcollectionView.reloadItems(at: [IndexPath(item: indexToUpdate, section: 0)])
                    self.scrollToBottom()
                }
            }
        }
    ...
    func scrollToBottom() {
        ...
        self.GJcollectionView.scrollToItem(at: lastItem, at: .bottom, animated: true)
    }
