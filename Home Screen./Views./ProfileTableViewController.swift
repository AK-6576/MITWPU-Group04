...
class ProfileTableViewController: UITableViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    // Variables to hold data passed from Home Screen
    ...
    override func viewDidLoad() {
            ...
            // 1. Logic for Name: Default to "Steve" if incomingName is nil or empty
            if let name = incomingName, !name.isEmpty {
                firstNameTextField.text = name
                firstNameTextField.textColor = .label // Standard Black
            } else {
                firstNameTextField.text = "Steve"
                // Use systemGray3 for a much lighter, placeholder-like look
            ...
            // 3. Setup Listeners (Auto-save AND Color change)
            setupTextFieldListeners()
        }
        
        func setupTextFieldListeners() {
            // A. Triggers when user types (Auto-save)
            firstNameTextField.addTarget(self, action: #selector(textFieldDidChange), for: .editingChanged)
            
            // B. Triggers the MOMENT the user taps the text field (Color change)
            firstNameTextField.addTarget(self, action: #selector(textFieldDidBeginEditing), for: .editingDidBegin)
        }

        // MARK: - TextField Actions
        
        // This runs as soon as the keyboard opens/user taps the field
        @objc func textFieldDidBeginEditing() {
            firstNameTextField.textColor = .label // Switches to Black immediately
        }

        @objc func textFieldDidChange() {
            performAutoSave()
            // Ensure it stays black while typing
    ...
    override func viewDidLayoutSubviews() {
        ...
        // Ensure circle layout happens after screen size is calculated
        setupProfileDesign()
    }
...
//    }
    ...
    func performAutoSave() {
        ...
        // 2. Send to Home Screen immediately
    ...
    @IBAction func saveButtonTapped(_ sender: Any) {
        // Redundant explicit save for safety
        performAutoSave()
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        if let finalImage = selectedImage {
            ...
            // 2. Trigger Update to Home Screen
            performAutoSave()
        }
